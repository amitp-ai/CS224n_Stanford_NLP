...
Author:
    ...
    Amit Patel: added method to get memory size info for SQuAD and updated description of the collate_fn() function
...
def get_model_info_old_delete(model):
    tot_num_parameters = 0
    tot_size = 0
    tot_size_MB = 0
    for name,param in model.named_parameters():
        if param.requires_grad:
            tot_num_parameters += 1
            tot_size += np.prod(param.shape)
            tot_size_MB += param.element_size() * param.nelement() / 1e6
            print(f'{name}, {param.shape}, param total dim: {param.nelement()}, {param.element_size() * param.nelement() / 1e6}MB')
    print(f'Total num of weights and biases {tot_size}, Total size {tot_size_MB}MB and Tot Num of parameters {tot_num_parameters}\n\n\n')
    # torchsummary.summary(model, [(90,), (90,30), (90,), (90,30)]) #in model's forward method, cast all inputs to long type


def cast_model_to(model, new_type):
    #modifies model inplace
    for l in model.modules():
        if True: #not isinstance(l, (torch.nn.modules.normalization.LayerNorm,)):
            l = l.to(dtype=new_type) #this is inplace operation (for nn.modules)
            # print(lnew is l)
...
class SQuAD(data.Dataset):
    ...
    def __init__(self, data_path, use_v2=True):
        ...
        self.valid_idxs = torch.tensor([idx for idx in range(len(self.ids))
                           if use_v2 or self.y1s[idx].item() >= 0])


    def move_to(self, device):
        self.valid_idxs.to(device)
        self.context_idxs.to(device)
        self.context_char_idxs.to(device)
        self.question_idxs.to(device)
        self.question_char_idxs.to(device)
        self.y1s.to(device)
        self.y2s.to(device)
        self.ids.to(device)
    ...
    def shuffle(self, new_size=None):
        ...
        with torch.no_grad():
            self.valid_idxs = self.valid_idxs[:new_size]
            idxs = torch.randperm(self.valid_idxs.shape[0])[:new_size]
            self.context_idxs = self.context_idxs[idxs]
            self.context_char_idxs = self.context_char_idxs[idxs]
            self.question_idxs = self.question_idxs[idxs]
            self.question_char_idxs = self.question_char_idxs[idxs]
            self.y1s = self.y1s[idxs]
            self.y2s = self.y2s[idxs]
    ...
    def memory_size(self):
        ...
        for v_name in variables:
            ...
            if v_name == 'valid_idxs':
                temp = sys.getsizeof(v)
            else:
                temp = v.element_size() * v.nelement()
            print('{}: {}MB'.format(v_name, temp/1e6))
            tot_mem += temp
        print('Total size: {}MB'.format(tot_mem/1e6))
...
def collate_fn(examples):
    ...
    all examples to the maximum length in the batch. input examples are already padded to the max limit, 
    but this function will only pad to the max seq length in the batch. Done for computational efficieny only.

    Args:
        examples (list): List of tuples of the form (context_idxs, context_char_idxs,
        question_idxs, question_char_idxs, y1s, y2s, ids). All of shape (batch_size, ...), where
        the remaining dimensions are the maximum limit in the input from args.
    ...
    Returns:
        ...
        the remaining dimensions are the maximum length of examples in the input batch.
...
def discretize(p_start, p_end, max_len=15, no_answer=False):
    ...
    # Compute pairwise probabilities of pstart*pend
    p_start = p_start.unsqueeze(dim=2) #(batch_size, c_len, 1)
    p_end = p_end.unsqueeze(dim=1) #(batch_size, 1, c_len)
